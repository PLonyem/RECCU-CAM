...
import { getLocalizedFields, localizeAnnouncement, localizeTrainingProgram } from "@/lib/localized-content";
...
export default async function AffiliatePortalSection({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === "circulars" || section === "notices") {
    ...
    const localizedRows = rows.map((row) => localizeAnnouncement(row, language));
    const title = section === "circulars" ? "Circulars" : "Official Notices";
    return <div className="space-y-8"><Header eyebrow="Official communications" title={title} description="Published RECCU-CAM communications available to your institution." />{localizedRows.length ? <div className="space-y-4">{localizedRows.map((row) => <Card key={row.id} className="p-5"><div className="flex flex-wrap items-center gap-2"><Badge>{row.category}</Badge><Badge>{statusLabel(row.priority)}</Badge></div><h2 className="mt-3 font-display text-xl font-semibold text-institutional">{row.title}</h2><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-600">{row.opening}</p><p className="mt-4 text-xs text-slate-500">Issued {formatDate(row.publishedAt)}</p></Card>)}</div> : <Empty title={`No ${title.toLowerCase()} available`} description="Official communications will appear here after publication." />}</div>;
  }

  if (section === "vtime") {
    const [programs, registrations] = await Promise.all([prisma.trainingProgram.findMany({ where: { published: true }, orderBy: [{ startDate: "asc" }, { title: "asc" }] }), prisma.trainingRegistration.findMany({ where: { affiliateId: affiliate.id }, include: { program: { select: { title: true, summary: true, translations: true } } }, orderBy: { createdAt: "desc" }, take: 10 })]);
    for (const program of programs) Object.assign(program, localizeTrainingProgram(program, language));
    for (const registration of registrations) Object.assign(registration.program, localizeTrainingProgram(registration.program, language));
