...
function SubmitButtons() {
  ...
  return <div className="sticky bottom-0 flex justify-end gap-3 border-t border-slate-200 bg-white/95 py-4">
    <button name="mode" value="draft" disabled={pending} className="rounded-lg border border-slate-300 px-5 py-2.5 font-semibold text-slate-700 disabled:opacity-60">{pending ? "Saving..." : "Save Draft"}</button>
    <a href="/" target="_blank" className="rounded-lg border border-slate-300 px-5 py-2.5 font-semibold text-slate-700">Preview published</a>
    <button name="mode" value="publish" disabled={pending} className="rounded-lg bg-institutional px-5 py-2.5 font-semibold text-white disabled:opacity-60">{pending ? "Saving..." : "Publish"}</button>
  </div>;
}

export function HomepageSectionsForm({ data }: { data: HomepageSectionsContent }) {
